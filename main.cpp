...
#if defined(NDEBUG)
# error "InfiniLooP cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "InfiniLooP Signed Message:\n";
...
{
    string strBlockFn = strprintf("il8p%04u.dat", nFile);
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    {
        try {
            CAutoFile il8pdat(fileIn, SER_DISK, CLIENT_VERSION);
            unsigned int nPos = 0;
            while (nPos != (unsigned int)-1 && il8pdat.good())
            {
                boost::this_thread::interruption_point();
                unsigned char pchData[65536];
                do {
                    fseek(il8pdat, nPos, SEEK_SET);
                    int nRead = fread(pchData, 1, sizeof(pchData), il8pdat);
                    if (nRead <= 8)
                    {
                        nPos = (unsigned int)-1;
                        break;
...
{
    RenameThread("infiniloop-loadil8p");
