...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("infiniloop"))
        return false;
...
{
    // Convert infiniloop:// to infiniloop:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("infiniloop://"))
    {
        uri.replace(0, 12, "infiniloop:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "InfiniLooP.lnk";
}
...
{
    return GetAutostartDir() / "infiniloop.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("InfiniLooP-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  infiniloop-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("InfiniLooP-Qt"));
...
{
    app.setStyleSheet("QWidget        { background: rgb(10,112,24); }"//this is the color of the entire right sided background - was 41,44,48
                      "QFrame         { border: none; }"
